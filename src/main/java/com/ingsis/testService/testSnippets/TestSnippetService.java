...
public class TestSnippetService {
  ...
  public TestSnippets updateTest(UpdateDTO dto) {
    ...
    for (String output : dto.outputs()) {
      existing
          .getExpectedOutputs()
          .add(new TestCaseExpectedOutput(UUID.randomUUID(), output, existing));
    }
  ...
  public void deleteTest(UUID testId) {
    ...
    TestSnippets testToDelete =
        testRepo.findById(testId).orElseThrow(() -> new EntityNotFoundException("Test not found"));
  ...
  public GetTestDTO convertToGetDTO(TestSnippets updated) {
    ...
    List<String> outputs =
        updated.getExpectedOutputs().stream().map(TestCaseExpectedOutput::getOutput).toList();
    logger.info("Creating GetTestDTO");
    return new GetTestDTO(
        updated.getId(), updated.getSnippetId(), updated.getName(), inputs, outputs);
  }
  ...
  public TestRunResultDTO runTestCase(TestToRunDTO testToRunDTO) {
    ...
    TestSnippets testCase =
        testRepo
            .findById(testToRunDTO.testCaseId())
    ...
    List<String> expectedOutputs =
        testCase.getExpectedOutputs().stream().map(TestCaseExpectedOutput::getOutput).toList();

    List<String> actualOutputs = simulateRun(inputs);

    boolean passed = actualOutputs.equals(expectedOutputs);
    String message =
        passed ? "Test passed" : "Expected: " + expectedOutputs + " but got: " + actualOutputs;

    return new TestRunResultDTO(
        passed ? TestStatus.PASSED : TestStatus.FAILED, message, actualOutputs, expectedOutputs);
  }
