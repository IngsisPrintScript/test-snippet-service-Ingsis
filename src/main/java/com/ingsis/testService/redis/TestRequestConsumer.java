        ...
        () -> {
          try {
            TestRequestEvent event =
                objectMapper.readValue(record.getValue(), TestRequestEvent.class);
            logger.info(
                "Processing lint request for Snippet({}) from User({})",
                event.snippetId().toString(),
                event.ownerId());

            TestRunResultDTO response =
                testSnippetService.runTestCase(new TestToRunDTO(event.testId(), event.snippetId()));
            testResultProducer.publish(
                new TestResultEvent(event.snippetId(), event.testId(), response.status()));
          } catch (Exception ex) {
